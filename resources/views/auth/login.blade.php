    ...
    <form method="POST" action="{{ route('login') }}">
        ...
        <div class="block mt-4">
            <label for="remember_me" class="inline-flex items-center">
                <input id="remember_me" type="checkbox" class="rounded border-gray-300 text-red-600 shadow-sm focus:ring-red-400" name="remember">
        ...
        <div class="flex flex-col items-end space-y-2 mt-4 sm:flex-row sm:items-center sm:justify-end sm:space-y-0">
            @if (Route::has('password.request'))
                <a class="underline text-sm text-orange-900 hover:text-orange-400 rounded-md focus:outline-none sm:self-auto" href="{{ route('password.request') }}">
