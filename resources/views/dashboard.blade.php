    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                ...
                <div id="snapshot-card-{{ $snapshot->id }}" class="group bg-white rounded-lg shadow-lg overflow-hidden hover:shadow-[0_0_25px_10px_rgba(251,146,60,0.4)]  transition-shadow duration-300">

                    <div class="relative">
                        <img src="{{ route('snapshots.image', $snapshot->id) }}" alt="{{ $snapshot->title }}"
                            class="w-full h-48 object-cover pt-3">

                        <div class="absolute inset-0 bg-transparent group-hover:bg-black/50 transition-all duration-300 flex items-center justify-center space-x-4">
                            ...
                            <div class="opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex space-x-3">
                                ...
                                <a href="{{ route('snapshots.download', $snapshot) }}" title="Download" download class="p-2 rounded-full bg-white hover:bg-gray-200">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-gray-700" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    <div class="p-4">
                        <div id="display-area-{{ $snapshot->id }}">
                            <div class="flex justify-between items-start">
                                <div>
                                    <h3 id="snapshot-title-{{ $snapshot->id }}" class="font-bold text-lg truncate">{{ $snapshot->title }}</h3>
                                    <p class="text-sm text-gray-500">{{ $snapshot->created_at->format('d M Y, H:i') }}</p>
                                </div>
                                <div class="flex space-x-2 flex-shrink-0">
                                    <button title="Ganti Judul" class="edit-button text-gray-400 hover:text-orange-400" data-snapshot-id="{{ $snapshot->id }}">
                                    ...
                                    <button title="Bagikan"
                                        class="share-button text-gray-400 hover:text-pink-500"
                        ...
                        <div id="edit-area-{{ $snapshot->id }}" class="hidden">
                            <div class="space-y-2">
                                <input type="text" value="{{ $snapshot->title }}" class="title-input w-full border border-gray-300 rounded-md shadow-sm px-3 py-2 focus:ring-indigo-500 focus:border-indigo-500">
                                <div class="flex justify-end space-x-2">
                                    <button class="cancel-button text-sm text-gray-600 hover:text-gray-900" data-snapshot-id="{{ $snapshot->id }}">Batal</button>
                                    <button class="save-button text-sm px-3 py-1 bg-blue-600 text-white rounded-md hover:bg-blue-700" data-update-url="{{ route('snapshots.update', $snapshot) }}" data-snapshot-id="{{ $snapshot->id }}">Simpan</button>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <div id="manual-share-popup" class="hidden fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
                <div class="bg-white rounded-lg shadow-xl p-6 w-full max-w-sm relative">
                    <button id="close-share-popup" class="absolute top-2 right-2 text-gray-500 hover:text-gray-800">&times;</button>
                    ...
                    <div class="flex justify-around items-center text-center">
                        ...
                        <a id="share-twitter" href="#" target="_blank" class="text-gray-700 hover:text-sky-500">
                            <svg class="w-10 h-10 mx-auto" fill="currentColor" viewBox="0 0 24 24">
                        ...
                        <a id="share-whatsapp" href="#" target="_blank" class="text-gray-700 hover:text-green-500">
                            <svg class="w-10 h-10 mx-auto" fill="currentColor" viewBox="0 0 24 24">
                    ...
                    <div class="mt-4 pt-4 border-t">
                        <input id="share-url-input" type="text" readonly class="w-full border-gray-300 rounded-md shadow-sm text-sm p-2 bg-gray-100">
                        <button id="copy-share-url" class="w-full mt-2 text-sm bg-gray-200 hover:bg-gray-300 rounded-md p-2">Salin Link</button>
                    </div>
                </div>
            </div>
        </div>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.body.addEventListener('click', function(event) {
                ...
                if (button.classList.contains('delete-button')) {
                    ...
                    if (confirm('Apakah Anda yakin ingin menghapus foto ini? Tindakan ini tidak bisa dibatalkan.')) {
                        ...
                        fetch(deleteUrl, {
                            ...
                            .then(data => {
                                if (data.success) {
                                    document.getElementById('snapshot-card-' + deleteUrl.split('/').pop()).remove();
                                } else {
                ...
                if (button.classList.contains('save-button')) {
                    ...
                    fetch(updateUrl, {
                        ...
                        .then(data => {
                            if (data.success) {
                                document.getElementById('snapshot-title-' + snapshotId).textContent = data.new_title;
                                document.getElementById('display-area-' + snapshotId).classList.remove('hidden');
                                document.getElementById('edit-area-' + snapshotId).classList.add('hidden');
                            }
                        })
